...
class StudentProfileField:
    ...
    def __init__(self, alias="", required=False, title="", default="", options=None, allow_non_suggested=False,
                 ...
                 minnumber=None, maxnumber=None, isnumber=None, isalnum=False, longtext=False):
        self.alias = alias
    ...
    def init_from_course(cls, course, default_fields=None):
        return cls.init_from_fields(course.credo_additional_profile_fields, default_fields=default_fields)

    @classmethod
    def init_from_fields(cls, profile_fields, default_fields=None):
...
def show_student_profile_form(request, course, simple_layout=False, redirect_to=None):
    ...
    if len(profiles) > 0:
        profile = profiles[0]
        saved_profile_fields = json.loads(profile.meta)
    else:
        saved_profile_fields = {}

    fields = StudentProfileField.init_from_course(course, saved_profile_fields)
    context = {
        'fields': fields.values(),
        'redirect_url': redirect_to if redirect_to else '',
        'form_submit_url': reverse('credo_modules_profile', kwargs={'course_id': str(course.id)})
    }
    if simple_layout:
        context.update({
...
def field_is_valid(value, field):
    if field.isnumber:
        try:
            value = int(value)
        except ValueError:
...
class StudentProfileView(View):
    ...
    def post(self, request, course_id):
        ...
        to_save_fields, errors = validate_profile_fields_post(request, course.credo_additional_profile_fields)
        if errors:
...
def validate_profile_fields_post(request, profile_fields):
    ...
    for field_alias, field in form_fields.items():
        ...
        if not passed_field and field.required:
            ...
            is_valid, err_msg = field_is_valid(data.get(field_alias, ''), field)
            if is_valid:
