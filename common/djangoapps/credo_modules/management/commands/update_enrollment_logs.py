...
from credo_modules.models import EnrollmentLog, TrackingLogProp, TrackingLogConfig
...
class Command(BaseProcessEnrollmentsLogsCommand):
    ...
    def handle(self, *args, **options):
        ...
        while process:
            ...
            print('Process CourseEnrollment items from %s to %s: ' % (str(dt_from), str(dt_to)))
        ...
        if new_last_log_time:
            print("Set 'last_enrollment_log_time' conf value: %s" % new_last_log_time.strftime('%Y-%m-%d %H:%M:%S.%f'))
            TrackingLogConfig.update_setting('last_enrollment_log_time', new_last_log_time.strftime('%Y-%m-%d %H:%M:%S.%f'))
