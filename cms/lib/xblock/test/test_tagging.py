...
class StructuredTagsAsideTestCase(ModuleStoreTestCase):
    ...
    def test_preivew_html(self):
        ...
        # Check that aside is rendered correctly
        self.assertRegexpMatches(problem_html, r'<select\s[_=a-zA-Z0-9\"\']+\sname=[\"\']%s[\"\']' % self.aside_tag)
        self.assertRegexpMatches(problem_html, r"data-block-type=[\"\']%s[\"\']" % self.aside_name)
        self.assertRegexpMatches(problem_html, r"data-init=[\"\']StructuredTagsInit[\"\']")
        self.assertRegexpMatches(problem_html, r"data-runtime-class=[\"\']PreviewRuntime[\"\']")
