...
class StructuredTagsAside(XBlockAside):
    ...
    def student_view_aside(self, block, context):  # pylint: disable=W0612,W0613
        """
    ...
    def save_tags(self, request=None, suffix=None):  # pylint: disable=unused-argument
        ...
        usage_key = None
        for path_part in request.path.split('/'):
            if path_part.startswith(AsideUsageKeyV1.CANONICAL_NAMESPACE):
                usage_key = UsageKey.from_string(path_part)
                break

        if usage_key:
            descriptor = modulestore().get_item(usage_key.usage_key)
            modulestore().update_item(descriptor, request._request.user.id, asides=[self])  # pylint: disable=protected-access
        else:
            return Response("The parent problem is not found", status=404)
