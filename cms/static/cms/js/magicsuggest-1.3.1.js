...
(function($)
{
    ...
    var MagicSuggest = function(element, options)
    {
        ...
        {
            var values = data, items = [], allowedValues = [], itemsNotInAllowed = [];
            if(!$.isArray(data)){
            ...
            $.each(_cbData, function(index, obj) {
                allowedValues.push(obj[cfg.valueField]);
            ...
            $.each(values, function(index, val) {
                if($.inArray(val, allowedValues) === -1) {
                    itemsNotInAllowed.push({id: val, name: val});
                }
            });

            if(itemsNotInAllowed.length > 0) {
                $.each(itemsNotInAllowed, function(index, itemNotInAllowed) {
                    items.unshift(itemNotInAllowed);
                });
            }
        ...
        var self = {
            ...
            _render: function(el) {
                ...
                if (w > 0) {
                    ms.input.width(w - (cfg.hideTrigger ? 16 : 42));
                }
            ...
            _renderSelection: function() {
                ...
                if(cfg.selectionPosition === 'inner') {
                    ...
                    if ((inputOffset > 0) || !cfg.value || ($.isArray(cfg.value) && (cfg.value.length == 0))) {
                        w = ms.container.width() - inputOffset - 42;
                        ms.input.width(w);
                    }

                    if (ms.selectionContainer.height()) {
                        ms.container.height(ms.selectionContainer.height());
                    } else {
                        ms.container.css('height', 'auto');
                    }
                }
